...
const SearchPage = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      let res = await axios.get(`/api/search/${activeTab}/${searchTerm}`);
      if (activeTab === "person") {
        const filteredActors = res.data.content.filter(
          (person) => person.known_for_department === "Acting"
        );
        setResults(filteredActors);
      } else {
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex justify-center gap-3 mb-4">
          <button
            ...
            onClick={(e) => handleTabClick("movie")}
          >
            Movies
          </button>
          <button
          ...
          <button
            ...
            onClick={(e) => handleTabClick("person")}
          >
            Person
          </button>
        </div>
